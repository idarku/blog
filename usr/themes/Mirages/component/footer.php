...
<?php
    $copyright = '<p><a href="'. Mirages::$options->siteUrl .'">今日无事</a>';
    if (Utils::hasValue(Mirages::$options->beian)) {
        $beiAn = "<a href=\"http://beian.miit.gov.cn\" target='_blank'> ".Mirages::$options->beian . "</a>";
        $copyright .= "</p><p>{$beiAn} • ";
    }
    if (Utils::hasValue(Mirages::$options->gonganwangbei)) {
        $wbIcon = STATIC_PATH . "images/gonganwangbei.png";
        if (Mirages::$options->gonganwangbeiNum__hasValue) {
            $wbNum = Mirages::$options->gonganwangbeiNum;
        } else {
            $wbNum = preg_replace('/[^0-9]+/i', '', Mirages::$options->gonganwangbei);
        }
        $wb = "<a href=\"http://www.beian.gov.cn/portal/registerSystemInfo?recordcode={$wbNum}\" target='_blank'><img src=\"{$wbIcon}\" height='13' alt='公安网备图标' style='vertical-align: text-top'> " . Mirages::$options->gonganwangbei . "</a>";
        $copyright .= "{$wb}</p><p>";
    }
    $copyright .= "{$theme}</p>";
?>
