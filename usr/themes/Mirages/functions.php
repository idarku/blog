...
require_once("lib/CollapseTitle${componentSuffix}.php");
require_once("lib/Content.php");
require_once("lib/Utils.php");
require_once("lib/Device.php");
require_once("lib/TOC.php");
require_once("lib/Lang.php");
require_once("lib/Comments${componentSuffix}.php");
require_once("lib/CategoryList${componentSuffix}.php");
...
function themeConfig(Typecho_Widget_Helper_Form $form) {
    ...
    $domainKey = new Typecho_Widget_Helper_Form_Element_Textarea('k', NULL, NULL, _mt('域名授权 Key'),_mt('域名校验码可以填写多个，每行填写一个（自动折行视为一行），其中一个与当前访问域名匹配即可通过。<br>域名校验 Key 可以到 <a href="https://store.get233.com/" target="_blank">主题小商店</a> 中我的订单处登记您的域名后获取。<br>如有任何疑问请直接联系作者。'));
...
//    $form->addInput($showBannerCurveStyle);
    ...
    $blogNotice = new Typecho_Widget_Helper_Form_Element_Textarea('blogNotice', NULL, NULL, _mt('博客公告消息'), _mt('显示在博客页面顶端的一条消息。' . $hrLine));
    ...
    $defaultThumbnails = new Typecho_Widget_Helper_Form_Element_Textarea('defaultThumbnails', NULL, '[center,top]{{MIRAGES_ROOT}}/images/default/katie-treadway-EwE4tBYh3ms-unsplash.jpg', _mt('卡片式文章列表的默认背景图列表'), _mt('在这里填入图片的URL地址, 每行填写一个链接（自动换行的视为同一行），在文章没有配置文章主图的时候，将会从这里挑选一张图片进行显示。' . $hrLine));
...
//    $form->addInput(new CollapseTitle('commentsDisqusTitle', NULL, NULL, _mt('评论 - Disqus'), ' <a href="https://get233.com/archives/mirages-disqus.html" target="_blank">'._mt('Disqus 评论及相关配置文档').'</a>'));
    ...
    $qiniuOptionsBlock = new Typecho_Widget_Helper_Form_Element_Checkbox('qiniuOptions',
        ...
        array(), _mt('云存储优化'), _mt('目前支持的云存储服务有：七牛云存储、又拍云存储、阿里云 OSS及腾讯云存储（需要开启<a href="https://cloud.tencent.com/product/ci" target="_blank">数据万象</a>功能），并依赖 Mirages 插件。'.Mirages::pluginAvailableMessage(103, '1.0.3') . $hrLine));
    ...
    $customCDNHosts = new Typecho_Widget_Helper_Form_Element_Textarea('customCDNHosts', NULL, NULL, _t('自定义域名的 CDN 类型配置（原插件配置：自定义 CDN 域名）'), _t('配置您的自定义域名使用的云存储服务类型，以便主题正确的识别云存储域名后，进行相应的优化，同时该项也是开启图像加载动画的必须配置项。<br>每行配置一个域名，自定义域名只要写域名即可，不需要写http/https.<br>配置方式为: <code>自定义域名 : CDN类型</code><br>例如：<code>cdn.example.com: QINIU</code><br>目前可以配置的CDN类型有：<br>七牛云存储「QINIU」<br> 又拍云存储「UPYUN」<br>阿里云 OSS「ALIOSS」<br>腾讯云存储-<a href="https://cloud.tencent.com/product/ci" target="_blank">数据万象</a>「QCLOUD」'));
    $form->addInput($customCDNHosts);
    $upYunSplitTag = new Typecho_Widget_Helper_Form_Element_Select('upYunSplitTag', array('!'=>_t('!'), '-'=>_t('-'), '_'=>_t('_')), '!', _t('又拍云间隔标识符'), _t('用于分隔图片 URL 和处理信息，可登录又拍云<a href="https://console.upyun.com/services/" target="_blank">控制台</a>，在 「服务」 > 「功能配置」 > 「云处理」 中设置。<br>又拍云的默认间隔标识符为「!」' . $hrLine));
    ...
    $beian = new Typecho_Widget_Helper_Form_Element_Text('beian', NULL, NULL, _mt('备案许可号'), _mt('未备案则不需要填，示例：京ICP备10000000号'));
    $form->addInput($beian);
    $gawb = new Typecho_Widget_Helper_Form_Element_Text('gonganwangbei', NULL, NULL, _mt('公安网备'), _mt('未备案则不需要填：示例：京公网安备11010000000000号<br>如果自动生成的链接指向不正确，可以通过高级设置：' . Utils::toCode('gonganwangbeiNum') . '来设置正确的 recordcode 参数' . $hrLine));
    $form->addInput($gawb);
    ...
    $copyright = new Typecho_Widget_Helper_Form_Element_Textarea('copyright', NULL, NULL, _mt('版权声明'), _mt('在这里填入你自己的版权声明内容。<br>你可以使用'.Utils::toCode("{{title}}").'表示文章标题，用'.Utils::toCode('{{link}}').'表示文章链接。') . $hrLine);
...
//    $layout->addItem($headTitle);
    $toc = new Typecho_Widget_Helper_Form_Element_Select('TOC', array('0'=>_mt('不启用'), '1'=>_mt('开启(默认隐藏)')), NULL, _mt('为这篇文章启用目录树'), NULL);
    $layout->addItem($toc);
    ...
    $enableSerifFonts = new Typecho_Widget_Helper_Form_Element_Select('useSerifFont', array('0'=>_mt('跟随主题设置'), '1'=>_mt('使用无衬线字体 (黑体)'), '2'=>_mt('使用衬线字体 (通过 WebFont 使用思源宋体)')), '0', _mt('字体风格选择'), _mt('选择该篇文章使用的字体风格，该选项会覆盖掉系统的全局设置。'));
    $layout->addItem($enableSerifFonts);
