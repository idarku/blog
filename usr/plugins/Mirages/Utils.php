...
class Mirages_Utils {
    ...
    static function isDevMode() {
        return defined("MIRAGES_DEVELOPER") && MIRAGES_DEVELOPER;
    }

    static function requireThemeFileIfNecessary($themeName, $themeFile, $className) {
        if (!empty($themeName)) {
            if (!class_exists($className)) {
                $file = Helper::options()->themeFile($themeName, $themeFile);
                if (file_exists($file)) {
                    if (!class_exists($className)) {
                        require_once($file);
                    }
    ...
    static function owoBasePackage($themeName) {

        $customOwOFile = Helper::options()->themeFile($themeName, "usr/biaoqing/OwO.json");
        if (file_exists($customOwOFile)) {
            return $customOwOFile;
        } else if (self::isDevMode()) {
            return Helper::options()->themeFile($themeName, "js/OwO.json");
        } else {

            if (!Mirages_Utils::requireThemeFileIfNecessary($themeName, '/lib/Mirages.php', "Mirages")) {
                return false;
            }
            return Helper::options()->themeFile($themeName, "js/" . Mirages::$version . "/OwO.json");
        }
    }
