...
class Mirages_Plugin implements Typecho_Plugin_Interface {
    ...
    private static function invokeContentPlugin($content, Widget_Abstract_Contents $widget) {
        $content = Typecho_Plugin::factory('Mirages_Plugin')->content($content, $widget);
        $content = Typecho_Plugin::factory('Mirages_Plugin')->content2($content, $widget);
    ...
    private static function invokeExcerptPlugin($content, Widget_Abstract_Contents $widget) {
        $content = Typecho_Plugin::factory('Mirages_Plugin')->excerpt($content, $widget);
        $content = Typecho_Plugin::factory('Mirages_Plugin')->excerpt2($content, $widget);
    ...
    private static function invokeContentExPlugin($content, Widget_Abstract_Contents $widget) {
        $content = Typecho_Plugin::factory('Mirages_Plugin')->contentEx($content, $widget);
        $content = Typecho_Plugin::factory('Mirages_Plugin')->contentEx2($content, $widget);
    ...
    private static function invokeExcerptExPlugin($content, Widget_Abstract_Contents $widget) {
        $content = Typecho_Plugin::factory('Mirages_Plugin')->excerptEx($content, $widget);
        $content = Typecho_Plugin::factory('Mirages_Plugin')->excerptEx2($content, $widget);
    ...
    private static function invokePostWritePlugin($content, Widget_Abstract_Contents $widget) {
        $content = Typecho_Plugin::factory("Mirages_Plugin")->writePost($content, $widget);
        $content = Typecho_Plugin::factory("Mirages_Plugin")->writePost2($content, $widget);
    ...
    private static function invokePageWritePlugin($content, Widget_Abstract_Contents $widget) {
        $content = Typecho_Plugin::factory("Mirages_Plugin")->writePage($content, $widget);
        $content = Typecho_Plugin::factory("Mirages_Plugin")->writePage2($content, $widget);
    ...
    const VERSION = 71003.99;
    const VERSION_TAG = "7.10.3";
...
HTML;
        ...
        $form->addInput(new Typecho_Widget_Helper_Form_Element_Hidden('speedTitle', NULL, NULL, _t('速度优化'), NULL));
        ...
        $customCDNHosts = new Typecho_Widget_Helper_Form_Element_Hidden('customCDNHosts', NULL, NULL, _t('自定义 CDN 域名'), _t('配置您的自定义域名使用的云存储服务类型，云存储的默认域名不需要填写。<br>每行配置一个域名，自定义域名只要写域名即可，不需要写http/https.<br>配置方式为: <code>自定义域名 : CDN类型</code><br>例如：cdn.example.com: QINIU<br>目前可以配置的CDN类型有：<br>七牛云存储「QINIU」<br> 又拍云存储「UPYUN」<br>阿里云 OSS「ALIOSS」<br>腾讯云存储-<a href="https://cloud.tencent.com/product/ci" target="_blank">数据万象</a>「QCLOUD」'));
        $form->addInput($customCDNHosts);
        $upYunSplitTag = new Typecho_Widget_Helper_Form_Element_Hidden('upYunSplitTag', array('!'=>_t('!'), '-'=>_t('-'), '_'=>_t('_')), '!', _t('又拍云间隔标识符'), _t('用于分隔图片 URL 和处理信息，可登录又拍云<a href="https://console.upyun.com/services/" target="_blank">控制台</a>，在 「服务」 > 「功能配置」 > 「云处理」 中设置。<br>又拍云的默认间隔标识符为「!」'));
        $form->addInput($upYunSplitTag);


        $form->addInput(new Typecho_Widget_Helper_Form_Element_Hidden('biaoqingTitle', NULL, NULL, _t('表情解析'), NULL));
        $biaoqingRootPath = new Typecho_Widget_Helper_Form_Element_Hidden('biaoqingRootPath', NULL, NULL, _t('自定义表情根目录'), _t('<span style="color: #F55852">其实完全可以不用配置</span><br>配置自定义的表情根目录，如果定义了就会使用自定义的路径加载表情，否则会使用<span style="color: #F55852">插件中自带的表情哦</span><br>每行配置一个表情路径，格式为：表情名称 (加英文冒号:) 自定义路径<br>示例：paopao:https://github.com/path/to/paopao/<br>目前可以配置的表情有：「paopao」、「aru」'));
    ...
    private static function addMiragesWidthAndSize($url) {
        ...
        if (in_array($currentCDNType, self::$supportedCDNType)) {
            ...
            if ($currentCDNType === self::CDN_TYPE_QINIU) {
                ...
                $splitTag = self::$themeOptions->upYunSplitTag;
                if (empty($splitTag)) {
                    $splitTag = "!";
                }
...
EOF;
    ...
    private static function loadPluginOptions() {
        ...
        self::$devMode = Mirages_Utils::isDevMode();
        ...
        $customCDNHosts = self::$themeOptions->customCDNHosts;
